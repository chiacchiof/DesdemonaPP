...
class DesBrain {
    ...
    static rankOperatorSelectionFacchini(input, allTaskFeatures, selectedOperators) {
        ...
        const {activityList, rankedOperatorsList} = DesBrain.evaluateCustomScenario(input, allTaskFeatures, selectedOperators);
    ...
    static rankOperatorSelection(input, allTaskFeatures, selectedOperators) {
        console.log('1. Method: rankOperatorSelection ---> Processing input:', input);
        console.log('2. Method: rankOperatorSelection ---> Processing allTaskFeatures:', allTaskFeatures);
        console.log('3. Method: rankOperatorSelection ---> Processing selectedOperators:', selectedOperators);
        const {activityList, rankedOperatorsList} = DesBrain.evaluateCustomScenario(input, allTaskFeatures, selectedOperators);
            ...
            checkedActivities[category].forEach(taskName => {
                ...
                if (taskFeatures) {
                    const operatorMatches = selectedOperators.map(operator => {
                        ...
                        const matchResult = DesBrain.calculateMatch(taskFeatures, operatorFeatures);
