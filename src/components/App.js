...
import { SaveOutlined, UploadOutlined, DownloadOutlined, PlusOutlined } from "@ant-design/icons";
...
import AddMaintenanceActivityModal from './AddMaintenanceActivityModal';
...
const App = () => {
  ...
  const handleAddActivitySuccess = (newActivity) => {
    if (!newActivity || !newActivity.title || !newActivity.tacom || !newActivity.tasks) {
      console.error('Invalid activity data received:', newActivity);
      return;
    }

    // Aggiorna la lista delle attività di manutenzione
    setConfig(prevConfig => {
      if (!prevConfig || !prevConfig.maintenanceActivities) {
        console.error('Invalid config structure:', prevConfig);
        return prevConfig;
      }

      // Trova il prossimo ID disponibile
      const existingKeys = Object.keys(prevConfig.maintenanceActivities);
      const nextId = `ACTIVITY_${existingKeys.length + 1}`;
      
      return {
        ...prevConfig,
        maintenanceActivities: {
          ...prevConfig.maintenanceActivities,
          [nextId]: {
            title: newActivity.title,
            tacom: newActivity.tacom,
            tasks: newActivity.tasks,
            status: 'pending', // Aggiungiamo uno stato iniziale
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString()
          }
        }
        ...
        <Row style={{ marginBottom: "10px" }}>
          <Col span={12}>
            <Select
              ...
              style={{ width: "100%" }}
            >
              {Object.keys(config.maintenanceActivities)
                .sort((a, b) => {
                  const titleA = config.ACTIVITY_DEFINITIONS[a] || a;
                  const titleB = config.ACTIVITY_DEFINITIONS[b] || b;
          ...
          <Col span={12} style={{ textAlign: 'right' }}>
            <Button
              type="primary"
              icon={<PlusOutlined />}
              onClick={() => setIsAddActivityModalVisible(true)}
            >
              Add New Activity
            </Button>
          </Col>
